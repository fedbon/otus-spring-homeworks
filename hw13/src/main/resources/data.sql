...
merge into users as target
    using (values
               ('adm', '$2a$12$8pnMmhZwwsHvAQNcti0rGe/w3L24vwBNbondgNEzYCMcoNi7XsRqS', 'ROLE_ADMIN'),
               ('usr', '$2a$12$RikQvoFNSmWkX1SbfWQ9QOu2gZElz.i8w84uNMcbfUpAAwzcXMHwe', 'ROLE_USER')
           )
        as source (username, password, authority)
    on target.username = source.username
    when not matched then
        insert (username, password, authority) values (source.username, source.password, source.authority);